...
export default function ServiceCentersAdmin() {
  ...
  return (
    <div className="space-y-3">
      <h1 className="text-2xl font-bold">Admin — Service centers</h1>
      <div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-3">No service centers yet.</td></tr>
            ) : (
              items.map(c => (
                <tr key={c.id} className="border-t">
                  ...
                  <td className="p-2 text-right space-x-2">
                    <button className="px-2 py-0.5 border rounded" onClick={()=>startEdit(c)}>Edit</button>
                    {c.id && <button className="px-2 py-0.5 border rounded text-red-600" onClick={()=>remove(c.id!)}>Delete</button>}
                  </td>
                </tr>
              ))
            )}
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
          <div className="bg-white p-4 rounded shadow max-w-lg w-full">
            <h2 className="font-semibold mb-3">{editing.id ? 'Edit' : 'Create'} service center</h2>
            <div className="grid gap-2">
              ...
              <input className="border px-2 py-1 rounded" placeholder="City" value={editing.city || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setEditing({ ...editing, city: e.target.value })} />
              <input className="border px-2 py-1 rounded" placeholder="Address" value={editing.address || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setEditing({ ...editing, address: e.target.value })} />
              ...
              <input className="border px-2 py-1 rounded" placeholder="Phone" value={editing.phone || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setEditing({ ...editing, phone: e.target.value })} />
